...
export default function NewFormationPage() {
  ...
  // Export image hook - simple approach, captures visible content
  const { captureRef, exportImage, isExporting } = useExportImage({
    fileName: `${formationName
      .toLowerCase()
  ...
  const handleExportImage = async () => {
    if (!formation) {
      alert("Không có dữ liệu đội hình để xuất ảnh.");
      return;
    }
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Không thể xuất ảnh");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 selection:bg-blue-600/30 transition-colors duration-200">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                title="Xuất đội hình dưới dạng ảnh PNG chất lượng cao"
              >
                <ImageIcon
                  size={14}
                  className="text-blue-500 dark:text-blue-400"
      ...
      >
        <FormationBoard captureRef={captureRef} />
      </main>
